...
test('Drip', () => {
  expect(Drip('')).toEqual('0');
  expect(Drip('0.0')).toEqual('0');
  expect(Drip('0x0a')).toEqual('10');
  expect(Drip(1e2)).toEqual('100');
  expect(new Drip(1e2)).toEqual('100');

  expect(() => Drip()).toThrow('Cannot');
  expect(() => Drip(null)).toThrow('Cannot');
  expect(() => Drip(-1)).toThrow('not match bigUInt');
  expect(() => Drip(3.14)).toThrow('cannot be converted to');
});
...
test('Drip.toXXX', () => {
  ...
  expect(drip).toEqual('3140000000');
